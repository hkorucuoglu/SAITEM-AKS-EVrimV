...
const float Radius = 0.15, PI = 3.14; //metre


struct MAIN
{
    unsigned long millis_counter, LastTime;
    unsigned int current_duty, pwm_period1, pwm_rate;
    float Circumference, totalTime, TotalDistance;
    ...
    void (*set_wiper)(void);
    void (*run_wiper)(void);
    void (*stop_wiper)(void);
    void (*hall_test)(void);
    void (*get_velocity)(void);
    unsigned long (*millis)(void);
    float (*get_circumference)(float);

};
type struct MAIN* Main;

Main init_main(); // constructor
...
float _get_circumference(float);
